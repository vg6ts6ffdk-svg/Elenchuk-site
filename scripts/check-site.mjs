...
import { pages, publicFiles } from '../public-files.mjs';
const root = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..');
const target = process.argv[2] ? path.resolve(root, process.argv[2]) : root;
const errors = [];
const docs = new Map(pages.map(file => [file, fs.readFileSync(path.join(target,file),'utf8')]));
...
for(const [file,html] of docs) {
  ...
  for(const match of html.matchAll(/\b(?:src|href|action)="([^"]+)"/g)) {
    ...
    references++;
    if(!publicFiles.includes(name) || !fs.existsSync(path.join(target,name))) errors.push(file + ': missing public file ' + name);
  ...
  for(const match of html.matchAll(/\bsrcset="([^"]+)"/g)) {
    for(const item of match[1].split(',')) {
      ...
      if(!publicFiles.includes(name) || !fs.existsSync(path.join(target,name))) errors.push(file + ': missing srcset ' + name);
    }
  }
  if(/<form\b/.test(html)) {
    ...
    if(file!=='admin.html' && !/<form[^>]*method="post"/.test(html)) errors.push(file + ': form must POST');
  }
...
for(const file of ['script.js','api-config.js','admin.js', ...(target===root?['server.js']:[])]) {
